...
function removeCustomer(indP) {
    ...
    insertCustomersInHTML();
    if (!multiCustomer)
        $("#customers-search-bar-mobile").show();
};


function insertCustomersInHTML() {
    $("#customers-container-mobile").html("");
    ...
    if (customersArray.length) {
        ...
        if (!multiCustomer) {
            ...
            $("#customers-search-bar-mobile").hide();
        }
        else {
            $("#customers-container-mobile").html(
                `
...
function getCustomersFromSearch() {
    $("#customers-search-warning-mobile").hide();
    $("#customers-search-noresult-mobile").hide();
    if ($("#customers-search-input-mobile").val().length < 3) {
        $("#customers-search-warning-mobile").show();
    }
    else {
        $.get("/api/customers/search/" + encodeURIComponent($("#customers-search-input-mobile").val()), function (results) {
            $("#customers-search-results-mobile").html("");
            if (results.customers.length) {
                $("#customers-search-input-mobile-clear-mobile").show();
                var __auxShowArr = [];
            ...
            else if ($("#customers-search-noresult-mobile").val().length < 3) {
                $("#customers-search-noresult-mobile").show();
            }
...
$(document).ready(function () {
    $("#customers-search-button-mobile").click(function () {
        getCustomersFromSearch();
    });

    $("#customers-search-input-mobile-clear-mobile").click(function () {
        $("#customers-search-results-mobile").html("");
        $("#customers-search-input-mobile-clear-mobile").hide();
    });

    let customersSearchInput = document.getElementById("customers-search-input-mobile");
