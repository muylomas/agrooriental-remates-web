...
window.addEventListener('load', function () {
    const autocompleteAddressElement = document.getElementById('autocomplete_address');
    if (autocompleteAddressElement != null) {
        var autocomplete = new google.maps.places.Autocomplete(autocompleteAddressElement);
        google.maps.event.addListener(autocomplete, 'place_changed', function () {
            ...
            $("[name='addressLatitude']").val(place.geometry.location.lat());
            $("[name='addressLongitude']").val(place.geometry.location.lng());
            update_marker_position(map, place.geometry.location.lat(), place.geometry.location.lng());
        });
...
function update_marker_position(map, mapLatitude, mapLongitude) {
    ...
    if ($("[name='autocomplete-marker-label']").val()) {
        mapLabel = $("[name='autocomplete-marker-label']").val();
    }
